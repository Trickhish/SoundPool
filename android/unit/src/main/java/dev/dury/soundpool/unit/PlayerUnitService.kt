...
import dev.dury.soundpool.unit.net.DeezerDataSource
import dev.dury.soundpool.unit.net.UnitSocket
 ...
 * what the server's conductor dictates (track, position, playing, volume),
 * streaming the audio and decrypting it on the fly.
 *
 * A foreground service on purpose — Android freezes background processes, which
 * would silently drop the room's output.
 */
@UnstableApi
class PlayerUnitService : Service() {

    companion object {
        private const val TAG = "SPUnit/svc"
    ...
    private fun onCommand(r: JSONArray) {
        when (r.optString(0)) {
            ...
            "error" -> if (r.optString(1) == "unknown_id") {
            ...
            // Audio routing (sinks/Bluetooth) is a PulseAudio concept on the
            // Linux units; the Android platform owns routing, so we report
            // nothing selectable rather than leaving the request unanswered.
    ...
    private fun doRender(song: JSONObject, url: String, key: String,
        ...
        // two-phase load, so reloading unconditionally would restart the track
        // at the exact moment it should begin.
    ...
    /** Cheap warm-up of the next track's CDN edge (DNS/TLS/first blocks). */
    private fun warmUp(url: String) {
        if (url.isEmpty()) return
        val req = Request.Builder().url(url)
            .header("Range", "bytes=0-${DeezerDataSource.BLOCK * 4 - 1}").build()
    ...
    private fun pushState() {
        val np = if (currentSongId == null) JSONObject.NULL else JSONObject().apply {
            put("id", currentSongId); put("title", currentTitle)
            put("artist", currentArtist); put("cover", currentCover)
    ...
    private fun buildNotification(text: String): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL)
            .setContentTitle("SoundPool speaker")
            .setContentText(text)
